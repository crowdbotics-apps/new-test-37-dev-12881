...
const nEWTESTAPI = axios.create({
  baseURL: "https://new-test-37-dev-12881-prod.herokuapp.com/",
  headers: { Accept: "application/json", "Content-Type": "application/json" }
})
...
function api_v1_test_list(action) {
  return nEWTESTAPI.get(`/api/v1/test/`)
}
function api_v1_test_create(action) {
  return nEWTESTAPI.post(`/api/v1/test/`, null, { data: action.data })
}
function api_v1_test_read(action) {
  return nEWTESTAPI.get(`/api/v1/test/${action.id}/`)
}
function api_v1_test_update(action) {
  return nEWTESTAPI.put(`/api/v1/test/${action.id}/`, null, {
    data: action.data
  })
}
function api_v1_test_partial_update(action) {
  return nEWTESTAPI.patch(`/api/v1/test/${action.id}/`, null, {
    data: action.data
  })
}
function api_v1_test_delete(action) {
  return nEWTESTAPI.delete(`/api/v1/test/${action.id}/`)
}
